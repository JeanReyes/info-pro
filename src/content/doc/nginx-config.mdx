...
title: Nginx
author: jean-reyes
description: Instrucciones básicas para configurar nginx
...
Enlace simbolico del nuevo sitio 
```sh 
...
tener acceso a en donde compraste el dominio `nic.cl`
En digitalOcean ir a networking y agregar dominio por defecto se agregarán los NS y estos ser registrados en nic.cl
Crear un nuevo registro para el dominio tipo A: @ 'para que sea el dominio raiz', y crear los subdominios correspondientes que debes tener configurados en tu droplet, ver pasos anteriores


##  Instalar app en node

Instalar node o mvn para isntalar diferentes versiones de node 
```sh
...
acceder por ssh a la IP del servidor 
```sh 
...
opcional se puede instalar `npm install pm2 -g` para manejar reinicios
arrancar app `pm2 start npm --name "docs.nes-sgd.cl" -- run start`, debes etsar en la carpeta del proyecto

para que sistema arranque `pm2 startup`, el comando proporciona un path target: `sudo env PATH=$PATH:/usr/bin pm2 startup systemd -u root --hp /root`
8. guardar estado actual `pm2 save`


## comando extra
eliminar archivo de droplet `sudo rm nombre_del_archivo`
eliminar archivo de pm2 `pm2 delete nombre_del_archivo`
restar nginx `sudo systemctl restart nginx`, pm2 `pm2 restart`
